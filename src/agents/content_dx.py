"""
Content-DX Agent — LangGraph v1
Doctrine §90: DX for Content domain.
Checks: notion-writer reachable, editorial workflows active in n8n, campaign_content_registry has rows.
"""
...
# Mogul Brief, Coffee Hour, Weekly Market Watch — production workflow IDs (could be env-configurable)
EDITORIAL_WORKFLOW_IDS = [w for w in (os.environ.get("CONTENT_DX_WORKFLOW_IDS", "").split(",") if os.environ.get("CONTENT_DX_WORKFLOW_IDS") else []) if w]
...
def run_health_checks(state):
    results = []
    # Check 1: notion-writer reachable
    ...
    # Check 2: campaign_content_registry has rows
    res2 = _supabase_get("campaign_content_registry?limit=5&select=campaign_key,step_index")
    has_rows = res2["status"] == 200 and isinstance(res2["body"], list) and len(res2["body"]) > 0
    results.append({"agent": "campaign_content_registry", "domain": "content", "tier": 2, "status": "healthy" if has_rows else "down", "raw": {"rows": len(res2["body"]) if isinstance(res2["body"], list) else 0}})
    # Check 3: n8n-writer reachable (so we can fire editorial workflows)
...
def escalate_if_failing(state):
    ...
    return {**state, "escalations": [{"channel": "telegram_admin", "result": _bridge_alert(f"Content-DX: {len(failures)} item(s) unhealthy:\n{bullets}", "warning", {"failures": failures})}]}
...
def summarize(state):
    ...
    return {**state, "summary": f"Content-DX: {healthy}/{total} healthy" + ("" if healthy == total else f", {escalated} escalation(s)")}
